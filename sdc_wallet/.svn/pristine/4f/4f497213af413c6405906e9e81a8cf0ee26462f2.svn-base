...
import com.utils.encode.AssertUtils;
...
public abstract class ServletUtils {
	 ...
	 * @param value
	 */
	public static void print(HttpServletResponse response, Object value) {
		if (logger.isDebugEnabled()) {
			logger.debug("print(HttpServletResponse, String) - start");
		}
...
//			response.setContentType("html/text;charset=utf-8");
			...
			logger.error("print(HttpServletResponse, String)", e);
		...
		if (logger.isDebugEnabled()) {
			logger.debug("print(HttpServletResponse, String) - end");
		}
	}

	public static String decodeParam(HttpServletRequest request,
			String paramName) {
		if (logger.isDebugEnabled()) {
			logger.debug("decodeParam(HttpServletRequest, String) - start"); //$NON-NLS-1$
		}

		String param = null;
		if (!StringUtils.isEmpty(request.getParameter(paramName))) {
			try {
				param = URLDecoder.decode(request.getParameter(paramName),
						"UTF-8");
			} catch (UnsupportedEncodingException e) {
				logger.warn(
						"decodeParam(HttpServletRequest, String) - exception ignored", e); //$NON-NLS-1$

			}
		}

		if (logger.isDebugEnabled()) {
			logger.debug("decodeParam(HttpServletRequest, String) - end"); //$NON-NLS-1$
		}
		return param;
	}
}
